...
Paper AntiXray во 2 или 3 режимах
:::
...
:::info Полезно
WinMTR - это бесплатная утилита с [открытым исходным кодом](https://github.com/White-Tiger/WinMTR), которая сочетает в себе функциональность `traceroute` и `ping`, позволяя получить детальную информацию о маршрутизации сетевых пакетов
:::
...
3. После запуска трассировки (кнопка "Start"), необходимо подождать (2-3 минуты будет достаточно), чтобы собрать статистику по каждому узлу на пути к серверу.
...
4. После проделанных действий, создайте тикет и предоставьте следующие данные:
    ...
    *   **Результат трассировки WinMTR (файл)**
    *   **ID сервера, на котором наблюдаются проблемы**

:::warning Внимание!
Без IP-адреса игрока предоставленные данные не будут иметь практической ценности.
Узнать свой внешний IP-адрес можно с помощью любого сервиса определения IP, например: **[2ip.ru](https://2ip.ru)**
:::